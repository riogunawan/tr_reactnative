...

{
	...
	protected $_script_foot = "";
	protected $_script_head = "";
	protected $_title = "";
	...
	private $_css = array();
	private $_inline_scripting = array(
		"infile" => "",
		"stripped" => "",
		"unstripped" => ""
	);
	...
	function css()
	{
		...
		foreach($css_files as $css_file) {
			...
			$is_external = false;
			if (is_bool($css_file)) continue;
			$is_external = preg_match("/^https?:\/\//", trim($css_file)) > 0 ? true : false;
			if (!$is_external)
			if (!file_exists($css_file)) show_error("Cannot locate stylesheet file: {$css_file}.");
	...
	public function script_foot($view, $parsingData = array())
	{
		$this->_script_foot .= "\n". $this->_set_script_foot($view, $parsingData);
		return;
	}
	
	public function script_foot_string ($js) {
		$this->_script_foot .= "\n
    ...
    private function _set_script_foot ($view, $parsingData = array()) {
        $ci = get_instance();
		$streamView = $ci->load->view($view, $parsingData, true);
		$script = "
            <script type='text/javascript'>
            	$streamView
            </script>
	...
	public function script_head($view, $parsingData = array(), $media = "screen")
	{
		$this->_script_head .= "\n" . $this->_set_script_head($view, $parsingData = array(), $media);
		return;
	}
	
	public function script_head_string ($css, $media = "screen") 
	{
		$this->_script_head .= "\n
			<style media='$media'>
				$css
    ...
    private function _set_script_head ($view, $parsingData = array(), $media) {
        $ci = get_instance();
		$streamView = $ci->load->view($view, $parsingData, true);
		$css = "
			<style media='$media'>
	...
	public function js()
	{
		...
		foreach($script_files as $script_file) {
			...
			$is_external = false;
			if (is_bool($script_file)) continue;
			$is_external = preg_match("/^https?:\/\//", trim($script_file)) > 0 ? true : false;
			if (!$is_external)
			if (!file_exists($script_file)) show_error("Cannot locate javascript file: {$script_file}.");
			$script_file = $is_external == FALSE ? base_url($script_file) : $script_file;
			if (!in_array($script_file, $this->_javascript)) $this->_javascript[] = $script_file;
		}
	...
	{
		if (!array_key_exists($area, $this->_sections)) $this->_sections[$area] = array();
		$CI = & get_instance();
	...
	function get_section($section_name)
	{
		...
		foreach($this->_sections[$section_name] as $section) $section_string.= $section;
	...
	*/
	...
	private
	function get_template_output($output)
	{
		if (function_exists("get_instance") && class_exists("CI_Controller")) {
			...
			$data["script_foot"] = $this->_script_foot;
			$data["script_head"] = $this->_script_head;
